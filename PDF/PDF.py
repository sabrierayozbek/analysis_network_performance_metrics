...
for i in range(line_number - 1):
	if float(times[i]) - counter <= 1:
		...
		if events[i] == 'r' and packets[i] == 'tcp':
			received_packet += int(packets_size[i])
			totaltime += float(times[i])
		...
		t.append(counter)
		counter += 1
		pdf = (received_packet / sent_packet)
		r.append(pdf)
		sent_packet = 0
		received_packet = 0
...
style.use('ggplot')
plt.title('Packet delivery fraction (PDF) Performans Grafiği')
plt.ylabel('Packet delivery fraction (PDF) Değerleri')
